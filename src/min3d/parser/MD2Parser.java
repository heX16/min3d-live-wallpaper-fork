...
public class MD2Parser extends AParser implements IParser {
	...
	private void getMaterials(BufferedInputStream stream, byte[] bytes) throws IOException {
		ByteArrayInputStream ba = new ByteArrayInputStream(bytes, header.offsetSkins - 68, bytes.length - header.offsetSkins);
		...
		for (int i = 0; i < header.numSkins; i++)
		{
			...
			skinPath = skinPath.substring(skinPath.lastIndexOf("/") + 1, skinPath.lastIndexOf("."));
			texture.append(skinPath.substring(skinPath.lastIndexOf("/") + 1, skinPath.lastIndexOf(".")));

			currentTextureName = texture.toString();
			Log.w("TAG", "Found texture " + currentTextureName);
	...
	private void getTexCoords(BufferedInputStream stream, byte[] bytes) throws IOException {
		ByteArrayInputStream ba = new ByteArrayInputStream(bytes, header.offsetTexCoord - 68, bytes.length - header.offsetTexCoord);
		LittleEndianDataInputStream is = new LittleEndianDataInputStream(ba);
	...
	private void getTriangles(BufferedInputStream stream, byte[] bytes) throws IOException {
		ByteArrayInputStream ba = new ByteArrayInputStream(bytes, header.offsetTriangles - 68, bytes.length - header.offsetTriangles);
		LittleEndianDataInputStream is = new LittleEndianDataInputStream(ba);
