	...
	private void parseObject(Object3d obj, HashMap<String, Material> materialMap, TextureAtlas textureAtlas) {
		...
		for (int i = 0; i < numFaces; i++)
		{
			...
			for (int j = 0; j < face.faceLength; j++)
			{
				...
				// : new Number3d();
				// Patch
				Uv newUv = face.hasuv && texCoords.size() > face.uv[j] ? texCoords.get(face.uv[j]).clone() : new Uv();
				Number3d newNormal = face.hasn && normals.size() > face.n[j] ? normals.get(face.n[j]) : new Number3d();
