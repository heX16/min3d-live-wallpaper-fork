...
/**
 * Convenient work-around for poor {@link FloatBuffer#put(float[])} performance. This should become unnecessary in gingerbread,
 * 
...
public class FastFloatBuffer {
	...
	/**
	 * Use a {@link SoftReference} so that the array can be collected if necessary
	 */
	...
	/**
	 * For use with pre-converted data. This is 50x faster than {@link #put(float[])}, and 500x faster than {@link FloatBuffer#put(float[])}, so if you've got float[] data that won't change, {@link #convert(float...)} it to an int[] once and use this
	 * method to put it in the buffer
	 * 
	 * @param data
	 *            floats that have been converted with {@link Float#floatToIntBits(float)}
	 */
	...
	/**
	 * Converts float data to a format that can be quickly added to the buffer with {@link #put(int[])}
	 * 
	 * @param data
